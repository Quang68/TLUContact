...
//        db = helper.getWritableDatabase();
    ...
    public interface StaffListCallback {
        void onCallback(List<CanBo> staffList);
    }

    public void fetchStaffList(CanBoDAO.StaffListCallback callback) {

        dbFireStore.collection("staff") // Truy vấn collection "students"
                .get()
                .addOnSuccessListener(snapshots -> {
                    ...
                    if (!snapshots.isEmpty()) {
                        for (QueryDocumentSnapshot document : snapshots) {
                            CanBo staff = new CanBo(
                                    document.getString("id"),
                                    document.getString("fullName"),
                                    document.getString("position"),
                                    document.getString("phoneNumber"),
                                    document.getString("email"),
                                    document.getString("departmant"),
